...
public class ProblemsOfUsingDirectSQLTests {
    ...
    *   이러한 수정 영향을 미치는것은 오류를 발생시킬 가능성도 있지만 유지보수성에도 악영향을 미친다.
    *   또한 객체를 사용할 때 SQL에 의존하게 되면 객체에 값이 무엇이 들어있는지 화긴해보기 위해 SQL을 매번 살펴봐야 한다.
    * */
    ...
    *   슈퍼타입의 모든 속성을 서브타입이 공유하지 못하여 물리적으로 다른 테이블과 분리가 된 형태이다.
    *   (설계에 따라서는 속성으로 추가되기도 함)
    *   하지만 객체지행의 상속은 슈퍼타입의 속성을 공유해서 사용하기 때문에 여기서 패러다임 불일치 현상이 발생하게 된다.
    * */

    /* 설명.
    *   벙인 회원과 일반 회원이라는 두 가지 회원을 추상화하여 회원이라는 슈퍼타입 엔터티를 도출하고
    ...
    *   public class Menu{
     ...
     * 설명.
     *  Menu menu= new Menu();
     *  Category category = new Category(); //메뉴와 카테고리의 관계설정
     *  menu.setCategory(category); //관계설정을 해주지 않으면 NPE 발생할 가능성 있음
     *
     * 설명.
     *  하지만 JPA에서는 정말 간단하게 이 문제가 해결된다.
     *  Menu menu = entityManager.find(Menu.class,menuCode);
     *  menu.getCategory().getCategoryName(); //NPE 발생하지 않음을 보장
    * */
